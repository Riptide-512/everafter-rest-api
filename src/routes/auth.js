...
auth.post('/login', async (c) => {
  ...
  const jwtSecret =
    c.env.JWT_SECRET ?? c.env.JWT_Secret ?? c.env.jwt_secret ?? c.env.JwtSecret
...
auth.post('/refresh', async (c) => {
  ...
  const accessToken = await signAccessToken({ sub: session.userId }, jwtSecret)
